...
export function applyRewardSignals(items = [], ctx = {}) {
  ...
  // Anchor item present — constraint satisfied, not hero worship ──────────
  if (ctx.anchorItem) {
    const anchorId = String(ctx.anchorItem.idx || ctx.anchorItem.id || "");
    const hasAnchor = items.some((it) => String(it.idx || it.id || "") === anchorId);
    if (hasAnchor)  { reward += 3; reasons.push("anchor piece included (constraint satisfied)"); }
...
// ── Intent Match Score ────────────────────────────────────────────────────────
// Primary signal: does this outfit actually EXPRESS the user's chosen vibe and
// occasion? A look that is "technically assembled" but fails to speak to the
// user's intent scores poorly regardless of how clean the silhouette is.
export function scoreIntentMatch(items = [], ctx = {}) {
  const nb      = itemBlob(items);
  ...
  const VIBE_SIGNALS = {
    minimal:           { strong: /\b(white|beige|cream|ivory|neutral|linen|trouser|loafer|turtleneck|clean|structured)\b/,  against: /\b(graphic|sequin|neon|embellished|streetwear|hoodie|loud)\b/ },
    "elevated casual": { strong: /\b(blazer|silk|cashmere|tailored|crisp|loafer|knit|oxford|polo)\b/,                       against: /\b(graphic|streetwear|gym|activewear|cargo)\b/ },
    sporty:            { strong: /\b(track|jogger|athletic|sneaker|bomber|zip.up|polo|stripe|active)\b/,                    against: /\b(heel|bodycon|gown|silk|sequin|jutti)\b/ },
    streetwear:        { strong: /\b(oversized|hoodie|sneaker|cargo|graphic|cap|bucket hat|jogger|chunky|baggy)\b/,         against: /\b(heel|pump|formal|silk|blazer|trouser|evening)\b/ },
    classic:           { strong: /\b(blazer|button.up|trouser|loafer|oxford|stripe|navy|white shirt|chino)\b/,             against: /\b(graphic|neon|cargo|athleisure|hoodie)\b/ },
    statement:         { strong: /\b(print|pattern|bold|embellished|sequin|color block|metallic|power)\b/,                 against: /\b()\b/ },
    "feminine soft":   { strong: /\b(ruffle|floral|pastel|pink|puff sleeve|lace|pearl|midi|ribbon|flowy|wrap|chiffon)\b/,  against: /\b(cargo|streetwear|graphic tee|hoodie|oversized)\b/ },
    "indo-western":    { strong: /\b(kurta|anarkali|palazzo|ethnic|embroidered|jutti|dupatta|salwar|lehenga|dhoti)\b/,     against: /\b()\b/ },
  };

  const OCC_SIGNALS = {
    "date night":       { strong: /\b(heel|dress|silk|fitted|elegant|blazer|bodycon|polished)\b/,           against: /\b(gym|activewear|cargo|hoodie|sneaker)\b/ },
    dinner:             { strong: /\b(heel|dress|silk|fitted|blazer|elegant)\b/,                            against: /\b(gym|cargo|hoodie|sweatshirt)\b/ },
    office:             { strong: /\b(blazer|trouser|loafer|oxford|button.up|midi|pencil skirt|smart)\b/,  against: /\b(sneaker|gym|activewear|shorts|crop top|hoodie)\b/ },
    brunch:             { strong: /\b(linen|midi|sundress|blazer|loafer|sneaker|relaxed|flowy)\b/,          against: /\b(gym|bodycon|gown|heavy formal)\b/ },
    travel:             { strong: /\b(sneaker|loafer|comfortable|knit|casual|practical|trouser|denim)\b/,   against: /\b(heel|bodycon|gown|festive)\b/ },
    airport:            { strong: /\b(sneaker|loafer|comfortable|knit|casual|practical|denim)\b/,           against: /\b(heel|bodycon|gown|festive|jutti)\b/ },
    festive:            { strong: /\b(ethnic|kurta|anarkali|lehenga|embellished|jutti|silk|embroidered)\b/, against: /\b(gym|sneaker|hoodie|cargo)\b/ },
    "wedding/event":    { strong: /\b(gown|silk|heel|blazer|formal|embellished|embroidered|lehenga)\b/,    against: /\b(gym|sneaker|hoodie|cargo|jogger)\b/ },
    vacation:           { strong: /\b(linen|sundress|sandal|flowy|casual|printed|breezy|kaftan)\b/,         against: /\b(formal|office|bodycon|gym)\b/ },
    everyday:           { strong: /\b(casual|denim|tee|sneaker|comfortable|everyday|loafer|knit)\b/,        against: /\b(gown|formal|embellished|bodycon)\b/ },
  };

  // Score vibe
  const vibeKey = Object.keys(VIBE_SIGNALS).find((k) => vibe.includes(k));
  if (vibeKey) {
    const { strong, against } = VIBE_SIGNALS[vibeKey];
    const hits    = (nb.match(new RegExp(strong.source, "gi"))  || []).length;
    const clashes = (nb.match(new RegExp(against.source, "gi")) || []).length;
    if      (hits >= 3) { score += 8; reasons.push(`strong ${vibeKey} vibe (${hits} signals)`); }
    else if (hits >= 1) { score += 4; reasons.push(`${vibeKey} vibe present`); }
    else                { score -= 2; reasons.push(`no ${vibeKey} vibe signals`); }
    if (clashes >= 1)   { score -= 4 * clashes; reasons.push(`vibe conflict: ${clashes} anti-${vibeKey} signal(s)`); }
  }

  // Score occasion
  const occKey = Object.keys(OCC_SIGNALS).find((k) => occ.includes(k));
  if (occKey) {
    const { strong, against } = OCC_SIGNALS[occKey];
    const hits    = (nb.match(new RegExp(strong.source, "gi"))  || []).length;
    const clashes = (nb.match(new RegExp(against.source, "gi")) || []).length;
    if      (hits >= 2) { score += 5; reasons.push(`strong ${occKey} occasion fit`); }
    else if (hits >= 1) { score += 2; reasons.push(`${occKey} occasion signals present`); }
    if (clashes >= 1)   { score -= 3 * clashes; reasons.push(`occasion conflict: ${clashes} anti-${occKey} signal(s)`); }
  }

  return { score, reason: reasons.join("; ") || "no intent signals matched" };
}
...
export function fullOutfitScore(items = [], ctx = {}) {
  ...
  const intent      = scoreIntentMatch(items, ctx);

  // Intent match is the primary signal — an outfit that doesn't express the
  // user's chosen vibe/occasion fails no matter how clean the silhouette is.
  // Anchor score is halved: it's a constraint check, not a virtue.
  const total =
    base.total +
    footwear.score    * 0.4 +
    penalties.penalty +
  ...
  const intentMismatch  = intent.score < -5;  // critical vibe/occasion conflict

  return {
    total: Math.round(total * 100) / 100,
    aesthetic,
    intentMismatch,
    accepted: !hasHardFail && !intentMismatch && total >= MINIMUM_SCORE,
    breakdown: {
      base: base.breakdown,
      footwear,
      penalties,
...
export function filterAndRankOutfits(outfits = [], ctx = {}, options = {}) {
  ...
  console.log(`[IntentHierarchy] Priority: 1.intent(${ctx.vibe || "any vibe"}/${ctx.occasion || "any"}) 2.weather 3.taste 4.anchor(constraint) 5.wardrobe`);
  ...
  if (ctx.anchorItem) {
    console.log(`[AnchorSupport]   Anchor: "${ctx.anchorItem.name || "item"}" acts as CONSTRAINT — outfit story is the hero`);
  }
  ...
  const scored = outfits.map((look, i) => {
    ...
    const intentReject = scoreResult.intentMismatch;
    const status = scoreResult.accepted ? "✅ PASS" : intentReject ? "🚫 INTENT MISMATCH" : "🚫 FAIL";

    console.log(`\n${status} OUTFIT ${i + 1} "${look.title || "Untitled"}" | score: ${scoreResult.total} | aesthetic: ${scoreResult.aesthetic}`);
    // Intent + vibe expression — primary signal logged first
    if (scoreResult.breakdown.intent) {
      const intentScore = scoreResult.breakdown.intent.score;
      console.log(`[VibeExpression]  "${look.title}" intent score: ${intentScore >= 0 ? "+" : ""}${intentScore} | ${scoreResult.breakdown.intent.reason}`);
      if (intentReject) {
        console.log(`[IntentMismatchRejected] "${look.title}" → outfit does not express chosen vibe/occasion (score:${intentScore}) — rejected`);
      }
    }
    console.log(`[OutfitStory]     "${look.title}" → aesthetic:${scoreResult.aesthetic} | anchor-support mode:${!!ctx.anchorItem}`);
    ...
    if (scoreResult.breakdown.anchor?.score !== 0 && scoreResult.breakdown.anchor?.reason)
      console.log(`[AnchorSupport]   [${scoreResult.breakdown.anchor.debug?.anchorType || "?"}] ${scoreResult.breakdown.anchor.reason} (${scoreResult.breakdown.anchor.score > 0 ? "+" : ""}${scoreResult.breakdown.anchor.score})`);
