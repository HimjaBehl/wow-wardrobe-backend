...
export async function updateTrends() {
  // Curated, stable fashion feeds (safe & India-relevant first)
  const feeds = [
    'https://www.vogue.in/feed/fashion.xml',
    'https://www.elle.com/rss/fashion.xml',
    'https://www.gq.com/about/fashion/rss',
    'https://wwd.com/fashion/feed/',             // Women’s Wear Daily
    'https://www.harpersbazaar.com/rss/all.xml'
  ];

  // 1) Pull headlines
  const allTitles = (await Promise.all(feeds.map(u => readFeedTitles(u))))
    .flat()
    .slice(0, 40);
  ...
  // 2) Ask AI to condense into structured trends
  const prompt = `
...
[
  {
    ...
    "season": "AW25",
    "score": 0.72   // 0..1 confidence of momentum
  }
]
- Use Indian dressing vocabulary when relevant (saree blouses, juttis, lehenga, kurta sets).
- Keep "keyword" punchy (2–3 words max).
- "content" <= 240 chars.
- Make "season" like "AW25" or "SS26" if implied; else "current".
- Prefer trends helpful for real outfits (silhouettes, palettes, key pieces).
  `;
  ...
  try {
    ...
    // If response_format enforced, content is a JSON string/object. Normalize:
  ...
  // 3) Save/update to Firestore: collection "trends"
  const col = collection(db, 'trends');
  ...
  // Optional: daily bucket for audits
  const dayKey = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  await setDoc(
    doc(collection(db, 'trend_runs'), dayKey),
    { count: trends.length, updated_at: now, feeds },
    { merge: true }
  );
...
// Function is now exported for use as API endpoint
// Uncomment below to run directly:
// updateTrends().catch(e => {
//   console.error('Trend update failed:', e.message);
//   process.exit(1);
// });
