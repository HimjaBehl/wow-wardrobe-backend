...
export function scoreWeatherFit(items = [], weatherStr = "") {
  ...
  // Heat-specific layer detection
  const hasHoodie       = /\bhoodie\b|\bsweatshirt\b|\bpullover\b/.test(nameblob);
  const hasDenimJacket  = /denim jacket|jean jacket|denim outer/.test(nameblob);
  const hasOuterJacket  = /\bjacket\b|\bcoat\b|\bblazer\b/.test(nameblob);
  // Double-layer: hoodie/sweatshirt worn under/over any jacket — always a heat trap above 24°C
  const isDoubleLayered = hasHoodie && hasOuterJacket;
  ...
  if (isHot) {
    ...
    if (hasHoodie)      { score -= 12; notes.push("hoodie/sweatshirt in 30°C+ heat — completely unwearable"); }
    else if (hasDenimJacket) { score -= 8; notes.push("denim jacket in scorching heat"); }
    else if (hasLayers) { score -= 5;  notes.push("layering in hot weather"); }
    if (isDoubleLayered){ score -= 8;  notes.push("double-layer in scorching heat — severe overheating risk"); }
    ...
    if (hasBoots)       { score -= 5;  notes.push("boots in 30°C+ heat"); }
  }

  if (isWarm) {
    if (/wool|fleece|velvet/.test(nameblob))    { score -= 4;  notes.push("wool/velvet in warm weather"); }
    if (hasHeavyLayer)                           { score -= 5;  notes.push("heavy coat in warm weather"); }
    if (hasHoodie)                               { score -= 8;  notes.push(`hoodie/sweatshirt at ${tempC}°C — too warm to layer`); }
    else if (hasDenimJacket)                     { score -= 5;  notes.push(`denim jacket at ${tempC}°C — warm weather mismatch`); }
    else if (hasLayers)                          { score -= 2;  notes.push("unnecessary outer layer in warm weather"); }
    if (isDoubleLayered)                         { score -= 10; notes.push(`double-layer at ${tempC}°C — hoodie + jacket is a heat trap`); }
  ...
  if (tempC !== null) {
    const cat = isHot ? "HOT" : isWarm ? "WARM" : isMild ? "MILD" : isChilly ? "CHILLY" : "COLD";
    console.log(`[WeatherScore] ${tempC}°C (${cat}) | hoodie:${hasHoodie} denim:${hasDenimJacket} heavyLayer:${hasHeavyLayer} doubleLayer:${isDoubleLayered} | score:${score}`);
  }
...
// Weather raised to 0.35 — practical wearability outweighs editorial styling
const WEIGHTS = { color: 0.15, silhouette: 0.20, weather: 0.35, occasion: 0.20, aesthetic: 0.10 };
