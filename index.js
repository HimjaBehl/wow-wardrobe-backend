  ...
  try {
    ...
    if (uid) {
      try {
        ...
        console.warn(
          "⚠️ Could not load user style context for /style-piece:",
          e.message,
        );
      }
    }
    ...
    const anchor = buildAnchorPieceContext({
      ...
      silhouette:
        ...
        guessSilhouette(
          `${anchor_item.name || ""} ${anchor_item.category || ""}`,
        ),
    });
    ...
    if (!combinedPool.length) {
      console.warn(
        "⚠️ /style-piece: Pool is empty (wardrobe:",
        wardrobeItems.length,
        "staples:",
        stapleItems.length,
        "). Proceeding with anchor only.",
      );
    }
    ...
    if (finalCandidateLooks.length < 3) {
      ...
      finalCandidateLooks = [...finalCandidateLooks, ...fallbackLooks];
    }

    // ✅ Final fallback if LLM gave nothing
    ...
    function normalizePieceRole(piece) {
      if (piece.role === "anchor" || piece.source === "uploaded_item")
        return piece;
      const cat = (piece.category || "").toLowerCase();
      let correctedRole = piece.role;
      if (
        /dress|jumpsuit|romper|one\s?piece|playsuit|saree|sari|gown|kaftan/.test(
          cat,
        )
      ) {
        correctedRole = "onepiece";
      } else if (
        /\bbag\b|purse|clutch|tote|handbag|backpack|satchel|crossbody/.test(cat)
      ) {
        correctedRole = "bag";
      } else if (
        /shoe|sneaker|boot|sandal|heel|footwear|slipper|loafer|mule|flat|pump|oxford|espadrille/.test(
          cat,
        )
      ) {
        correctedRole = "footwear";
      } else if (
        /pant|jeans|skirt|short|trouser|legging|palazzo|chino|cargo\s?pant|culottes/.test(
          cat,
        )
      ) {
        correctedRole = "bottom";
      } else if (
        /\bshirt\b|blouse|tee\b|t-shirt|\btop\b|tank|crop|cami|polo|henley|tunic|kurta|bustier|corset|bodysuit/.test(
          cat,
        )
      ) {
        correctedRole = "top";
      } else if (
        /jacket|coat|blazer|cardigan|hoodie|sweater|shrug|vest|poncho|windbreaker|parka|overcoat|trench/.test(
          cat,
        )
      ) {
        correctedRole = "outer";
      } else if (
        /earring|necklace|bracelet|ring|watch|scarf|hat|belt|sunglasses|accessory|jewel|tiara|brooch|hair\s?clip|bandana/.test(
          cat,
        )
      ) {
  ...
  try {
    ...
    function weightedSample(arr, max, opts = {}) {
      ...
      const scored = arr.map((it) => {
        ...
        const colorStr = Array.isArray(it.color)
          ? it.color[0] || ""
        ...
        const isBlazer =
          /blazer|suit|jacket/.test(name) || /blazer|suit|jacket/.test(catg);
        const isTrouser =
          /trouser|pants|chinos|slacks/.test(name) ||
          /trouser|pants|chinos|slacks/.test(catg);
        const isSandal = /sandal|flip/.test(name) || /sandal|flip/.test(catg);
        const isClosedToe =
          /loafer|oxford|derby|heel|pump|boot/.test(name) ||
          /footwear/.test(catg);
    ...
    function buildSampleFromList(list = [], max = 50) {
      ...
      return subset.map((it, idx) => {
        ...
        const silhouetteGuess =
          it.silhouette || guessSilhouette(`${cleanName} ${cleanCat}`);
    ...
    const FEWSHOTS = [
          ...
          outfits: [
            {
              ...
              style_note:
                "Neutral tee with denim and clean sneakers for easy balance.",
          ...
          outfits: [
            {
              ...
              style_note:
                "Navy dress with nude heels; simple silhouette and soft contrast.",
              items: [{ idx: "3" }, { idx: "4" }],
            },
          ],
        }),
          ...
          outfits: [
            {
              ...
              style_note:
                "Pastel kurta + palazzo + juttis; airy fabrics and cohesive palette.",
...

      {
        ...
        content: JSON.stringify({
          ...
          dislikedCombos: [
            "top-neutral-upper|bottom-bright-lower|footwear-neutral-footwear",
          ],
          ...
          outfits: [
            {
              ...
              style_note:
                "Avoided prior disliked combo; chose warm top, tapered chinos, minimal sneakers.",
   ...
   Always include Footwear when possible.
      ...
      content: JSON.stringify(
        {
          ...
          instructions: [
            ...
            ...(lastServedCombo
              ? [`Avoid repeating this combo fingerprint: ${lastServedCombo}`]
              : []),
    ...
    if (!finalAssistantContent) {
      ...
      try {
        ...
        const jsonMatch =
          finalAssistantContent && finalAssistantContent.match(/\{[\s\S]*\}/);
        if (jsonMatch) {
          try {
            ...
            console.log(
              "🧼 Sanitized (recovered) AI payload (head):",
              JSON.stringify(recovered).slice(0, 400),
            );
          } catch (e2) {
        ...
        if (recovered) {
          ...
          dlog(
            "⚠️ Could not parse assistant JSON. Returning weighted fallback.",
          );
          const pool =
            wardrobeSample && wardrobeSample.length >= 6
              ? wardrobeSample
      ...
      parsed.outfits = await Promise.all(
        (parsed.outfits || []).map(async (look, i) => {
          ...
          const catsLower = hydrated.map((h) =>
          ...
          if (!STRICT_ITEMS && dislikedCombos.includes(combo)) {
            ...
            if (!PRESERVE_STYLE_TEXT) {
              look.style_note =
                (look.style_note || "") +
                " ⚠️ Similar to a previously disliked combination.";
            }
          }
          ...
          if (!STRICT_ITEMS && likedCombos.includes(combo)) {
            ...
            if (!PRESERVE_STYLE_TEXT) {
              look.style_note =
                (look.style_note || "") + " ❤️ Based on your favorites!";
            }
          }
          ...
          if (uppers.length && lowers.length) {
            ...
            const bothLoose =
              uppers.every((it) =>
                /loose|oversize/.test(it.silhouette || ""),
              ) &&
          ...
          function basicCompletenessCheck(look) {
            const cats = look.items.map((it) =>
              (it.category || "").toLowerCase(),
            );

            // Path A: Top + Bottom + Footwear
            const hasTop = cats.some((c) =>
              /top|shirt|tee|t-?shirt|blouse|kurta/.test(c),
            );
            const hasBottom = cats.some((c) =>
              /bottom|jeans|pants|trouser|skirt|shorts|palazzo|salwar/.test(c),
            );
            const hasFootwear = cats.some((c) =>
              /footwear|shoe|sandal|heel|sneaker|jutti|boot/.test(c),
            );
            const hasOnePiece = cats.some((c) =>
              /dress|jumpsuit|saree/.test(c),
            );

            const onlyAccessories = cats.every((c) =>
              /accessor|sunglass|watch|bag|belt|scarf|dupatta|stole|shawl/.test(
                c,
              ),
            );
          ...
          if (!STRICT_ITEMS) {
            if (!basicCompletenessCheck({ items: hydrated })) {
              ...
              const isOnePiecePath = cats.some((c) =>
                /dress|jumpsuit|saree/.test(c),
              );
          ...
          if (!STRICT_ITEMS && isInvalid) {
            console.log(
              "⚠️ Look validation failed - self-healing disabled in this mode",
            );
            // Self-healing can be added here if needed
          }

          const changedCategories = (() => {
            const before = (look.items || [])
              .map((it) => (it.category || "").toLowerCase())
              .sort()
              .join(",");
            const after = hydrated
              .map((it) => (it.category || "").toLowerCase())
              .sort()
              .join(",");
            return before !== after;
      ...
      parsed.outfits = parsed.outfits.map((l) => {
        if (!l.validation?.styleRules?.valid) {
          ...
          console.warn(
            "❌ Look failed validation, asking Tina to retry:",
            errs,
          );
        ...
        parsed.outfits = (parsed.outfits || []).map((look) => {
          ...
          if (!PRESERVE_STYLE_TEXT) {
            if (!look.style_note?.toLowerCase().includes(lowerOccasion)) {
              look.style_note =
                `${look.style_note || ""} Styled for ${occasion}.`.trim();
            }
          }
          return look;
        });
      ...
      console.log(
        "🎯 Sending outfits:",
        (response.outfits || []).map((o) => o.items?.length),
      );
